...
class Market(Page):
    ...
    form_model = 'player'
    form_fields = ['nbida','bida','naska','aska','nbidb','bidb','naskb','askb']
   ...
   # def naskc_error_message(self, value):
    ...
    def error_message(self, values):
        ...
        if values["bida"]*values["nbida"]+values["bidb"]*values["nbidb"] > self.player.cash:
...
class Clearing(WaitPage):
    ...
    def after_all_players_arrive(self):
        ...
        for p in players: 
            ...
            p.group.d_c=0
            p.t_c=0
            p.delta_cash = decimal.Decimal(-1*p.group.p_a*p.t_a-1*p.group.p_b*p.t_b)
            p.di_a = p.group.d_a*(p.n_a + p.t_a)
            p.di_b = p.group.d_b*(p.n_b + p.t_b)
            p.di_c = p.group.d_c*(p.n_c + p.t_c)
            
            p.pay_display= p.di_a + p.di_b
        
            if self.subsession.round_number==Constants.num_rounds or self.subsession.round_number==Constants.rounds_per_market:
                p.pay_display +=  (p.n_a + p.t_a)*self.session.config['pfinal'][0] +(p.n_b + p.t_b)*self.session.config['pfinal'][1] + p.cash + p.delta_cash
...
class Adios(Page):
    ...
    def vars_for_template(self):
        ...
        return {
            ...
            'points_indollars': round(
                self.player.cum_payoff_topay * self.session.config['real_world_currency_per_point'], 2),
        }
